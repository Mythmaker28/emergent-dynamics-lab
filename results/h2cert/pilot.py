...
OUT="/tmp/h2"; BUDGET=float(os.environ.get("BUDGET","38")); t0=time.time()
...
weng=mk(C.TRACER)
man=json.load(open(os.environ["REPO"]+"/results/dmm01/dmm01_prospective_manifest.json")); HH=man["dev"]["histories"]; SEEDS=[37001,37002]
def load(p,d): return pickle.load(open(p,"rb")) if os.path.exists(p) else d
def save(p,o): pickle.dump(o,open(p,"wb"))
warm=load("/tmp/dmm/warm.pkl",{})
...
CHECK=[0,300,500,650,800]
D=load(f"{OUT}/pilot.pkl",[]); seen={(r["seed"],r["hi"]) for r in D}
for hi,(a_e,a_l) in enumerate(HH):
    for seed in SEEDS:
        if (seed,hi) in seen: continue
        if not tleft(): save(f"{OUT}/pilot.pkl",D); print("BUDGET",len(D)); sys.exit()
        st=relabel(apply2(warm[seed],a_e,a_l));pc=mk(PulseChaseTracer());rb={}
        for step in range(CHECK[-1]+1):
            if step in CHECK: rb[step]=feat(st)
            st=pc.step(st)
        D.append({"seed":seed,"hi":hi,"h2":a_l-a_e,"traj":rb}); save(f"{OUT}/pilot.pkl",D)
print("COMPLETE pilot n=%d"%len(D))

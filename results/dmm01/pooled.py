...
weng=mk(C.TRACER)
man=json.load(open(os.environ["REPO"]+"/results/dmm01/dmm01_prospective_manifest.json")); HH=man["dev"]["histories"]
w1=pickle.load(open("/tmp/smc/warm.pkl","rb")); w2=pickle.load(open(f"{OUT}/warm.pkl","rb"))
WARM={**{k:w1[k] for k in (36001,36002) if k in w1},**{k:w2[k] for k in (37001,37002) if k in w2}}
SEEDS=list(WARM)
...
D=load(f"{OUT}/pooled.pkl",[]); seen={(r["seed"],r["hi"]) for r in D}
for hi,(a_e,a_l) in enumerate(HH):
    for seed in SEEDS:
        if (seed,hi) in seen: continue
        if not tleft(): save(f"{OUT}/pooled.pkl",D); print("BUDGET",len(D)); sys.exit()
        st=relabel(apply2(WARM[seed],a_e,a_l));pc=mk(PulseChaseTracer());rb={}
        for step in range(CHECK[-1]+1):
            if step in CHECK: rb[step]=feat(st)
            st=pc.step(st)
        D.append({"seed":seed,"hi":hi,"h2":a_l-a_e,"h1":a_e+a_l,"traj":rb}); save(f"{OUT}/pooled.pkl",D)
print("COMPLETE pooled n=%d"%len(D))

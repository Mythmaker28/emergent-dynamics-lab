...
def summarize_distances(rec: dict) -> dict:
    ...
    if "U" in rec:
        out["d_U"] = float(np.linalg.norm(rec["U"]["R"] - Re))
    if "C" in rec:
        dc = [float(np.linalg.norm(r - rec["H"]["R"])) for r in rec["C"]["R_clones"]]
        out["clone_ceiling_mean"] = float(np.mean(dc))
        out["clone_ceiling_max"] = float(np.max(dc))
    return out

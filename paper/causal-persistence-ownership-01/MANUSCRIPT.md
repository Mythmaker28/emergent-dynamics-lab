...
For each target, the own effect is intact uptake minus uptake after its own memory is erased. The neighbour effect is the average change caused by erasing either other target. The sham effect is the change after sham erasure. We average the three target contrasts within each original world before inference. Thus {{N_TARGET}} targets provide {{N_VALID}} world-level observations, not {{N_TARGET}} independent replicates.
...
Of {{N_WORLD}} worlds, {{N_INELIGIBLE}} lacked three initially eligible targets. {{N_SPLIT}} further worlds encountered a split and {{N_LOST}} a lost track before reaching the turnover threshold. The remaining {{N_VALID}} worlds reached the threshold and passed both assay requirements. No additional deep-reaching world was excluded by assay geometry. Figure 1 accounts for all generated worlds.

Across the {{N_TARGET}} retained targets, own-marked material fractions ranged from {{M_MIN}} to {{M_MAX}} (rounded), all at or below the specified threshold. Deep snapshots occurred {{DEEP_MIN}}–{{DEEP_MAX}} steps after marking. These observations demonstrate the specified change in material composition in admissible targets, not complete replacement of all material or continuity of an independently established identity.
...
The own-erasure effect was {{OWN}} integrated uptake units, with a world-level 95% t interval of {{OWN_CI}}. Mean intact uptake was {{INTACT_MEAN}} units. The mean fractional reduction, calculated for each target and then averaged within and across worlds, was {{OWN_RELATIVE_PERCENT}}%; world means ranged from {{OWN_RELATIVE_MIN}}% to {{OWN_RELATIVE_MAX}}%. The absolute effect was positive in every retained world. The own-minus-sham and own-minus-neighbour contrasts were also positive, with means {{OWN_MINUS_SHAM}} and {{OWN_MINUS_NEIGHBOUR}} respectively (Figure 2). These branch comparisons support a causal effect of intervening on the represented target field, within the selected population and standardized assay. Together with decoding, they do not establish that the causal component of the field specifically mediates the assigned history; erasure also removes any non-history-specific component.

The effect remained positive when observation masks were held fixed during each branch's probe: {{FIXED_WITHIN_BRANCH}}, interval {{FIXED_WITHIN_BRANCH_CI}}. This convergent direction does not eliminate region-selection differences between branches. Under uptake-channel ablation, the residual own-erasure effect was {{PLUS_ABLATION_RESIDUAL}}, interval {{PLUS_ABLATION_RESIDUAL_CI}}. The residual was nonzero, consistent with the retained attractant channel and coupled dynamics. The frozen causal gate passed.

An additional paired check gives 0.5 × own effect − residual = {{HALF_OWN_MINUS_RESIDUAL}}, interval {{HALF_OWN_MINUS_RESIDUAL_CI}}, positive in all retained worlds. This post hoc comparison supports attenuation below half the intact effect without treating the estimated intact mean as a fixed denominator. The descriptive reduction in the ratio of mean effects was {{ATTENUATION_PERCENT}}%. None of these comparisons identifies copying, ongoing writing or an error-correcting mechanism as the unique cause of the retained field.
...
|---|---:|---:|
| N: nearest-target memory | {{ADV_N}} | {{ADV_N_BAND}} |
| E: memory-masked radial fields | {{ADV_E}} | {{ADV_E_BAND}} |
| Gm: memory-masked global fields | {{ADV_GM}} | {{ADV_GM_BAND}} |
| B: body/field baseline | {{ADV_B}} | {{ADV_B_BAND}} |
...
With complete refitting after deleting one original world, E and B each crossed the historical lower-band threshold in {{REFIT_PASS_E}} and {{REFIT_PASS_B}} of the {{N_REFIT}} deletions, respectively. Gm lost its original threshold passage in {{REFIT_FAIL_GM}} deletions. These exploratory checks show that the gate pattern is sensitive to a small number of worlds and to refitted prediction models. They strengthen the case for reporting the unmet conjunction as limited evidence rather than converting it into an ontological negative.

All {{N_INPUT}} packaged input files passed byte and hash checks. The additional primary reconstruction agreed with the historical raw-only implementation to a maximum absolute difference of {{MAX_REPRO_DIFF}} for checked causal summaries and access bands. The independent prediction implementation also reproduced the permutation result. These are independent calculations on the same observations. They are not replications on new seeds or external validation of the simulator's scientific constructs.
...
Our result is deliberately conditional. The world-level exclusion rule prevents treating surviving targets within an invalid world as independent successes, but it also selects a restricted geometric population. {{N_INELIGIBLE}} worlds were initially ineligible and {{N_TRACK_CENSORED}} lost continuous tracking before the turnover criterion. Effects in these worlds were not measured under the same valid assay. Their missing outcomes cannot be assigned zero or extrapolated from the retained mean. Formal assay validity also depends on post-intervention geometry, although it produced no further exclusions among deep-reaching worlds here.

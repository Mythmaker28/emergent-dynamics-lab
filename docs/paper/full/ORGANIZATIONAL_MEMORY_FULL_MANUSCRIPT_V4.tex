...
\date{July 2026 \quad (V4: headline longitudinal numbers adopt the committed reproducible pipeline; see HEADLINE\_NUMBER\_ERRATUM)}
\begin{document}
\maketitle
...
(longitudinal hold--out on untouched seeds: $h_1$ held--out $R^2=0.89$, $95\%$ CI $[0.84,0.96]$ at old--material
fraction $M\approx0.19$, with $36/36$ track survival), $h_1$ is causally expressed in the interventional sense that the memory--inert and erase controls carry no history information (deterministically), with a positive but statistically uncertain sufficiency magnitude (transplant $R^2=0.61$, $95\%$ CI $[-0.69,0.87]$ at $n=12$); its \emph{retention} through deep turnover is the load--bearing result ($R^2=0.89$, $95\%$ CI $[0.84,0.96]$), from prospective held--out trajectories analysed with a retrospectively standardized, fully committed analysis pipeline.
Crucially $h_1$ is \emph{global} in informational content: it is decodable from essentially any large droplet, and
therefore does \textbf{not} individuate droplets. \textbf{Negative:} a second initially decodable coordinate $h_2$ (temporal order / value dispersion) has an $\approx$one--dimensional causal response, and its deep--turnover retention is \emph{not established}: the held--out estimate ($R^2=-0.24$) carries a wide $95\%$ confidence interval ($[-0.78,0.32]$) whose upper bound is below the $0.50$ qualification threshold, so at $n=12$ histories the coordinate does not meet the certification criterion. Our methodological contribution is a
...
($\lambda_+\!\to\!0$ or $\lambda_-\!\to\!0$), and an exact--clone / stochastic ceiling (Table~\ref{tab:interventions}). \textbf{Canonical analysis.} All longitudinal held--out numbers reported below are produced by the committed, deterministic pipeline \texttt{python -m reproduction.primary} (module \texttt{reproduction/}); these are prospective, held--out trajectories analysed with a retrospectively standardized, fully committed pipeline. The earlier inline scoring that produced the historical values is not recoverable and is documented in \texttt{HEADLINE\_NUMBER\_ERRATUM.md}.
...
$h_2$ is highly family--dependent, and a preregistered kill--switch on a sealed family, executed once, returned a deep--turnover held--out $h_2$ near $0$ at $M\approx0.21$ under the original per--frame tracker; after the tracker--continuity correction (\S\ref{sec:tracker}), the canonical committed pipeline gives deep--turnover $h_2=-0.24$ at $M\approx0.19$ with a $95\%$ CI $[-0.78,0.32]$ (and $-0.03$, $[-0.68,0.57]$, at the shallower $M\approx0.39$) (Figure~\ref{fig:killswitch}); the interval lies below the $0.50$ threshold, so retention is \emph{not established}. Following the preregistered stopping rule, no further $h_2$ mechanism development was undertaken on this architecture (the criterion for continuing was not met). Before closing, we decomposed the loss to avoid mislabelling its cause. Contrary to an intuitive expectation, disabling the two spatial smoothing terms (templating and diffusion) individually or together left the $h_2$ decode trajectory essentially unchanged, and the memory value--dispersion in fact \emph{grows} during turnover rather than homogenizing; a dedicated unit test confirmed the ablations were physically active (they alter the one--step memory field) but negligible on the decode because growth injects competing dispersion. Nor is the loss dilution: old and new material decode $h_2$ equally at intermediate turnover, because new material inherits the local memory. Earlier pooled analyses had suggested partial $h_2$ retention at intermediate turnover with high between--family variance; under the canonical committed pipeline the order coordinate is already below threshold by moderate turnover ($-0.03$ at $M\approx0.39$) and remains so at deep turnover. The canonical value is reported in Table~\ref{tab:results}.
...
survival) and certifies $h_1$: initial $R^2=0.78$ ($[0.56,0.94]$), deep $R^2=0.89$ ($[0.84,0.96]$) at $M\approx0.19$ (the paper's primary quantitative result),
while $h_2$ remains $-0.24$ (the confidence interval is over $12$ donor--group histories, not individual rows), ($<0.50$) (Figure~\ref{fig:cert}, Table~\ref{tab:tracker}). Critically, $h_1$ decodes comparably from the largest ($0.91$) and the longitudinally--tracked entity ($0.89$),
...
$h_1$ initial (longitudinal hold--out) & $0.78$ & $[0.56,0.94]$ & decodable\\
$h_1$ deep turnover, longitudinal (hold--out) & $0.89$ & $[0.84,0.96]$ & \textbf{certified}\\
...
$h_1$ memory vs size+mass (sealed family, $M{\approx}0.21$) & $0.93$ vs $0.64$ & CIs overlap & suggestive\\
$h_2$ initial (longitudinal hold--out) & $0.80$ & $[0.53,0.92]$ & decodable\\
$h_2$ deep turnover, longitudinal & $-0.24$ & $[-0.78,0.32]$ & \textbf{not established}\\
...
largest--at--each--frame (historical) & $0.91$ & --- & reselection; teleports\\ longitudinal (corrected, canonical) & $0.89$ & $-0.24$ & $0$ switches, $36/36$ survive\\
...
\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig_longitudinal_v4.png}
...
$\times 3$ seeds, executed once. $h_1$ held--out $R^2$ with $95\%$ CI versus $M$ (deep $0.89$ [$0.84,0.96$] at $M\approx0.19$); $h_2$ below threshold ($-0.24$ [$-0.78,0.32$]); $100\%$ track survival ($36/36$, $0$ switches). Prospective held--out trajectories; donor--level bootstrap CIs. Generated by \texttt{python -m reproduction.primary} from \texttt{results/observer/tca\_holdout\_raw.pkl}.}
\label{fig:cert}\end{figure}

\begin{table}[t]\centering\small
\caption{\textbf{Canonical longitudinal synthesis} --- generated verbatim by \texttt{python -m reproduction.primary}
(\texttt{synthesis\_table.tex}); no value is typed by hand. Grouped leave--one--history--out ridge decode of the
memory features versus turnover fraction $M$, with donor--level bootstrap $95\%$ CIs.}
\label{tab:synthesis}
\input{synthesis_table.tex}
\end{table}
...
\begin{figure}[t]\centering\includegraphics[width=0.80\textwidth]{figs/fig_h2_deepturnover_v4.png}
\caption{\textbf{$h_2$ deep--turnover (canonical pipeline).} Held--out $h_2$ decode versus surviving original--material fraction $M$ with $95\%$ bootstrap CIs; dashed line is the $0.50$ qualification threshold and the shaded band is below it. At deep turnover the canonical longitudinal $h_2=-0.24$ has CI $[-0.78,0.32]$, lying below the threshold, so deep--turnover retention is \emph{not established} --- this bounds the C1c architecture and this dispersion metric, and is not a demonstrated absence of all order information nor a substrate--level impossibility. Generated by \texttt{python -m reproduction.primary} from \texttt{results/observer/tca\_holdout\_raw.pkl}.}
\label{fig:killswitch}\end{figure}

\begin{figure}[t]\centering\includegraphics[width=0.9\textwidth]{figs/fig_gate_summary_v4.png}
\caption{\textbf{Canonical gate summary.} Deep--turnover held--out decode and gates from the committed pipeline: $h_1$ certified ($0.89$ [$0.84,0.96$]), $h_2$ not established ($-0.24$ [$-0.78,0.32$]), $36/36$ survival, $0$ switches; individuation not supported. Generated by \texttt{python -m reproduction.primary}.}
...
$h_1$ deep--turnover (longitudinal) & held--out $R^2\ge0.50$, lower CI${>}0$ & pass ($0.89$, $[0.84,0.96]$)\\
$h_2$ deep--turnover & held--out $R^2\ge0.50$, lower CI${>}0$ & not established ($-0.24$, CI $[-0.78,0.32]$)\\
...
$h_1$ deep--turnover survival & replaced: longitudinal $0.89$, $[0.84,0.96]$, $36/36$\\
...
$h_2$ turnover failure & unchanged ($-0.24<0.50$ longitudinally)\\\bottomrule
\end{tabular}
\end{table}
\begin{table}[t]\centering\small
...
kill--switch $h_2$ not established deep ($-0.24$, CI $[-0.78,0.32]$) & bounds C1c architecture & absence of an order memory\\\bottomrule
\end{tabular}
\end{table}
...
substantial constituent turnover ($h_1$ held--out $R^2=0.89$, $[0.84,0.96]$, at $M\approx0.19$, $36/36$ survival),

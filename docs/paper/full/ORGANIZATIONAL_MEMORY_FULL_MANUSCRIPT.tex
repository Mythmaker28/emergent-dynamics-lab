...
production ($m_-=m_1-m_2$). Over a preregistered ladder of interventions evaluated prospectively, with grouped
...
fraction $M\approx0.19$, with $36/36$ track survival), and $h_1$ is causally expressed through uptake and
transplantable into a fresh standardized body ($R^2=0.61$), exceeding body--size/mass baselines after turnover.
Crucially $h_1$ is \emph{global} in informational content: it is decodable from essentially any large droplet, and
therefore does \textbf{not} individuate droplets. \textbf{Negative:} a second initially decodable coordinate $h_2$
(temporal order / value dispersion) fails qualification --- its causal response is $\approx$one--dimensional, and its
deep--turnover held--out $R^2=0.34$ ($<0.50$) under continuity--safe tracking. Our methodological contribution is a
...
At the biological end, our droplet is a caricature of an active droplet or protocell. Chemically active droplets can
...
protocells can encapsulate, grow, and divide \citep{szostak2001,hanczyc2003}. This literature largely concerns
\emph{physical} self--organization and reproduction; it does not typically ask whether such droplets carry a
\emph{causal, turnover--surviving memory of their individual experience}, which is our question. Molecular biology
...
second coordinate should not survive turnover, and it points to bistable maintenance as the missing ingredient. The observability lens clarifies why the two coordinates behave so differently under turnover \citep{hermann1977}. The cumulative coordinate $h_1$ is encoded in the \emph{level} of the memory, a quantity that growth and uniform death preserve and that the smoothing terms do not erase; it is therefore robustly observable through turnover. The order coordinate $h_2$ is encoded in the \emph{dispersion} of the memory, a quantity that linear low--pass maintenance and fresh, history--independent growth actively degrade; it is therefore observable at rest but not through deep turnover. This is not a subtle statistical accident but a structural property of the maintenance operator, and it is exactly the property that a bistable, hysteretic rule --- which protects a discrete distinction rather than averaging a continuous one --- would change \citep{gardner2000,ferrell2002,ptashne2004}. In
...
$M=\text{mass}_{\text{old}}/\text{mass}_{\text{total}}$ of the tracked entity quantifies turnover; the pulse--chase
tracer is passive and never enters $\rho,U,V,N,c$ \citep{jaqaman2008}. Interventions comprise erasure ($M_f=0$),
...
components are labelled with a periodic (wrap--aware) algorithm and summarized by circular centroids
...
Longitudinal metrics are reported only for uncensored tracks, so a lost entity is never silently replaced by another.
...
threshold under alternatives $R^2\in\{0.50,0.65,0.75\}$. When the pilot point estimate itself lies at or below the
threshold, no finite sample can raise the lower bound above it; adequate power then means enough samples to show the
effect is \emph{below} threshold with confidence, which is the kill--switch's designed function. The frozen
prospective sample was fixed accordingly, and the family was executed once. \paragraph{The experimental sequence.} The results below are the outcome of nine preregistered experiments. (1) A material--continuity experiment established that the droplet persists under turnover but that a snapshot nearly suffices to predict its future, i.e.\ no strong history--specific hidden persistence. (2) A hidden--state experiment showed the internal state is a generic causal attractor, not an individuating fingerprint. (3) An organizational--memory experiment introduced the designed two--component field and showed it causal, erasable, transplantable, with a readout that collapsed onto one cumulative coordinate. (4) A multi--channel experiment added the $m_-\!\to$attractant readout and made temporal order causally readable. (5) A writing--dimensionality audit corrected an apparent high--dimensional storage via grouped evaluation. (6) A writing redesign (C1c) produced two decodable coordinates at rest but a one--dimensional causal response. (7) A spatial--memory experiment characterized $h_2$ as a frame--free dispersion and first reported a turnover collapse. (8) A maintenance experiment corrected that report --- the collapse was family--dependent and underpowered, the loss neither homogenization nor dilution --- but did not certify deep survival. (9) A final kill--switch certified, on a sealed family, that $h_2$ does not survive deep turnover. The tracker--continuity audit then re--certified all turnover claims longitudinally.
...
turnover--related. Figure~\ref{fig:causal} shows the held--out $h_1$ causal response after mean transplantation into
a fresh body ($R^2=0.61$), the in--place memory--isolated effect ($R^2=0.50$), and the memory--inert control, which
collapses the response --- the interventional signature of a causal memory \citep{pearl2009}. The magnitude of the transplant effect ($0.61$) is smaller than the rest--state storage
decode ($0.94$) precisely because the mean--field transplant discards spatial structure and reads only the scalar
entity means; the point is not the magnitude but the interventional dissociation between the active and inert
conditions, which no residual environment or body--size confound can produce (the recipient body is standardized and
erased).
...
to the writing gains and timescales (the frozen ``C1c'' configuration; Table~\ref{tab:params}) yields two
prospectively decodable coordinates \emph{at rest} ($h_1,h_2\approx0.94$ held--out), with the second--dimension
...
survival) and certifies $h_1$: initial $R^2=0.92$ ($[0.86,0.99]$), deep $R^2=0.98$ ($[0.97,1.00]$) at $M\approx0.19$,
while $h_2$ remains $0.34$ ($<0.50$) (Figure~\ref{fig:cert}, Table~\ref{tab:tracker}). Critically, $h_1$ decodes
...
$h_1$ causal (mean transplant / in--place) & $0.61$ / $0.50$ & --- & established\\
$h_1$ vs body baseline at $M{\approx}0.21$ & $0.93$ vs $0.64$ & --- & established\\
$h_2$ at rest (sealed) & $0.90$ & $[0.41,0.97]$ & decodable\\
$h_2$ deep turnover, longitudinal & $0.34$ & spans $0.5$ & \textbf{fail} ($<0.50$)\\
$h_2$ causal (transplant / in--place) & $-0.04$ / $0.30$ & --- & fail\\
...
\begin{figure}[t]\centering\includegraphics[width=0.86\textwidth]{figs/fig1.png}
...
\begin{figure}[t]\centering\includegraphics[width=0.55\textwidth]{figs/fig2.png}
\caption{\textbf{$h_1$ is causally expressed and transplantable.} Protocol: mean transplant of donor memory into a
fresh standardized erased body, in--place memory--isolated response (full minus memory--inert), and the
memory--inert control. Prospective, grouped leave--history--out, $n=12$ histories. Held--out $h_1$ response
$R^2=0.61$ (transplant), $0.50$ (in--place); the memory--inert control collapses it (causal necessity). Uncertainty:
donor--level. Limitation: mean--field readout. Data: \texttt{results/wd01\_phasec/phasec\_causal\_*\_raw.pkl};
script \texttt{causal\_runner.py}.}
\label{fig:causal}\end{figure}

\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig3.png}
...
\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig3b.png}
\caption{\textbf{Longitudinal hold--out certification.} Untouched sealed seeds $38502$--$38504$, $n=12$ histories
...
\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig4.png}
...
\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig5.png}
...
\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig6.png}
\caption{\textbf{$h_2$ deep--turnover kill--switch.} Sealed family executed once: held--out $h_2\approx0$ at
$M\approx0.21$ with $48/48$ viability; family--dependence and moderate--turnover variability. Prospective. Data:
\texttt{results/h2cert/h2cert\_sealed\_raw.pkl}.}
\label{fig:killswitch}\end{figure}

\begin{figure}[t]\centering\includegraphics[width=0.95\textwidth]{figs/fig7.png}
\caption{\textbf{Falsification ladder and final gate summary.} Intervention ladder (qualified / correction /
negative / kill--switch) and the final gate table: $h_1$ qualifies on every gate, $h_2$ does not, individuation
fails. Source: \texttt{docs/paper/paper\_claim\_ladder.png}.}
...
\label{tab:gates}
\begin{tabular}{@{}lll@{}}\toprule
gate & criterion & outcome\\\midrule
...
$h_2$ deep--turnover & held--out $R^2\ge0.50$ & fail ($0.34$)\\
...
\begin{tabular}{@{}lll@{}}\toprule
observation & licenses & does not license\\\midrule
...
kill--switch $h_2\approx0$ deep & architecture bound & substrate--general impossibility\\\bottomrule
\end{tabular}
\end{table}
...
such protection, and an adequately powered kill--switch confirmed that $h_2$ does not survive deep turnover. This is
a clean, informative negative: it delimits what the tested architecture can support and identifies the specific
ingredient (protected, nonlinear maintenance) a future architecture would need.
...
budding, or lineage} experiment, and make no heredity or genome claim. Ninth, the second--dimension rank ratio at
prospective evaluation ($0.283$) is marginally below the preregistered gate ($0.30$); we report this as a strict,
marginal miss. Tenth, the tracker, though frozen and label--free, was developed with knowledge of one incident
...
and expresses it causally and transplantably. This memory does not individuate droplets: its informational content
is global even though storage and causal expression are local. A second, initially decodable coordinate did not
qualify as an independent, causal, turnover--surviving dimension, and was closed by a preregistered kill--switch.
...
preserved. No data were withheld.
...
$h_1$ certification survives this scrutiny because its lower bound is far from the threshold, whereas the $h_2$
failure is confirmed rather than manufactured by the small sample.

...
## 8. Frozen red-team corrections (`RED_TEAM_RECONCILIATION_03A.md`, `turnover_ownership_analyze.py`)

1. **Neighbour = geometrically nearest droplet** (minimum periodic centroid distance), NOT `(i+1)%K`. The parent
   (i+1)%K number (neigh 0.58 > own 0.135) was a labelling artefact; with geometric neighbours own > neighbour.
2. **Leak-free world bootstrap**: LOGO predictions computed once on unique worlds; bootstrap resamples worlds over
   fixed held-out (y, prediction) pairs — a duplicated world can never enter both train and test (the naive
   relabel-group bootstrap leaked in 91 % of resamples and inflated the CI). Frozen.
3. **λ₊-only ablation** (`MEM_ABLATE_PLUS`, λ₋ kept) is the PRIMARY manipulation check; the both-zero ablation is
   reported separately. DEV: λ₊-only leaves ~10 % residual via the attractant (λ₋) channel — reported, not hidden.
4. Primary own-dose decode uses the 11-D target memory features; L/P/E/G scopes (persisted by the prospective
   runner) drive the ACCESS comparison. All decoders/regularization/nulls frozen here; no post-hoc selection.

...
Preregister and launch EXP-RD-01: a BLIND low-discrepancy map over (F, k, Du, Dv), matched OPEN vs the EXACT CLOSED limit (F=k=0), on the qualified open RD stack. Five evidence levels kept separate; frozen P/M and thresholds; no composite; no visual selection. Any screening permission must pass: frozen fresh-seed hold-out -> alias audit (incl. the imposed-pattern/occupancy check) -> same-state CONTROL/SHAM/PERTURBED/PLACEBO causal intervention WITH the continued-turnover requirement -> observer (cadence/tracker) sensitivity -> adversarial re-audit. Report explicit denominators, censored seeds, effect sizes and Wilson CIs.

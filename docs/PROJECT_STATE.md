...
USER DECISION. The closed, globally mass-conservative Flow-Lenia system (core + reservoir-exchange throughput) is EXHAUSTED for this question: it produces constituent turnover but no sustained, constituent-carried individuality that survives multi-seed causal replication and observer sensitivity. RECOMMENDED next substrate: a genuinely OPEN system (reaction-diffusion with true sources/sinks and non-equilibrium driving), preregistered under the SAME falsifiable protocol (qualified stack, passive tracers, five evidence levels, frozen P/M, continued-turnover requirement, placebo-controlled same-state causal intervention, observer-sensitivity checks). Do NOT add further mechanisms to the closed conservative system, relax thresholds, or form composites.

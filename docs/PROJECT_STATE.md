...
EXP-FL-03: replicate the same-state causal intervention for laws {2,16,40,59} on SEVERAL fresh causal seeds under the frozen protocol (CONTROL/SHAM/PERTURBED/PLACEBO, sham==control required); verify the re-established structure at the new site CONTINUES to turn over (not a frozen lump); run an independent adversarial re-audit (hostile reviewer pass) and a field-stack cadence/tracker sensitivity check. Only after all of these may anything be considered for promotion. Do NOT relax P/M, add a composite, or select laws visually.

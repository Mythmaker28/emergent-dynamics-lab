...
EXP-RD-02: run the frozen downstream chain for the 9 OPEN permitted laws {1,5,7,10,11,13,14,16,19}, IN ORDER: (1) frozen fresh-seed hold-out on unseen seeds, retain only at >=2/5; (2) direct alias audit INCLUDING the imposed-pattern/occupancy check (a structure reappearing at the OLD site after displacement is an externally maintained spatial slot, not individuality) and the dissolution/look-alike check; (3) same-state CONTROL/SHAM/PERTURBED/PLACEBO causal intervention WITH the continued-turnover requirement (a re-established frozen lump is NOT a success) on many unseen causal seeds per law; (4) observer (cadence/tracker) sensitivity on every audited unit; (5) adversarial re-audit. Report explicit denominators, censored seeds, audited successes, alias-compatible cases, placebo failures, frozen-lump failures, effect sizes and Wilson CIs. Do NOT relax thresholds, add composites, or select laws visually.

...
def main():
    ...
    # depth is len(commits) + 1 so that the OBDCA01 head is a REAL commit in the archive and
    ...
    subprocess.run(["git", "-C", bare, "remote", "remove", "origin"], env=ENV,
                   capture_output=True)
    tar = f"{D}/OBTR01_OFFLINE_REPO.tar.gz"
    ...
    manifest = [
        "OBTR01 — ORGANIZER-BOUND-TIMESCALE-REDERIVATION-01",
        "branch  %s" % BRANCH,
        "head    %s" % head,
        "tree    %s" % tree,
        "base    %s (OBDCA01)" % BASE,
        ...
        "",
        "    cat OBTR01_OFFLINE_REPO.tar.gz.part* > OBTR01_OFFLINE_REPO.tar.gz",
        "    sha256sum -c whole.sha256",
        "    tar xzf OBTR01_OFFLINE_REPO.tar.gz",
    ...
    rec = {
        "SECTION": "OBTR01 §29 — delivery record",
        ...
        "ARTEFACT": {"whole_sha256": whole, "parts": len(parts),
                     ...
                     "boundary_is_the_OBDCA01_head": shallow == BASE},
        ...
        "APPEND_ONLY_NOTE": ("this mission modifies no inherited artefact. The OBDI02 and "
                             ...
                             "OBTR01/out/OBTR01_APPEND_ONLY_NOTES.md."),
    }
    ...
    for ln in (err or out).splitlines()[:4]:
        print("   %s" % ln)
    print()
    print("archive %s, %d parts, boundary %s (is OBDCA01 head: %s)"

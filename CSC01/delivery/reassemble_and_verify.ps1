...
param([string]$Out = "$PWD\orr01_offline")
$here = Split-Path -Parent $MyInvocation.MyCommand.Path
New-Item -ItemType Directory -Force -Path $Out | Out-Null
$parts = Get-ChildItem "$here\ORR01_OFFLINE_REPO.tar.gz.part*" | Sort-Object Name
Write-Host "== reassembling from $($parts.Count) parts"
$dst = Join-Path $Out "ORR01_OFFLINE_REPO.tar.gz"
if (Test-Path $dst) { Remove-Item $dst }
$fs = [System.IO.File]::Create($dst)
foreach ($p in $parts) { $b = [System.IO.File]::ReadAllBytes($p.FullName); $fs.Write($b, 0, $b.Length) }
$fs.Close()
$expected = (Select-String -Path "$here\ORR01_OFFLINE_REPO.SHA256SUMS" -Pattern 'reassembled whole').Line.Split(' ')[0]

...
SEED_BASE, SEEDS_PER = 910000, 9
AXORD = ("+x", "-x", "+y", "-y")
CAD, SHADOW_EVERY = 16, 128
KAPPA, REG_FLOOR, BACKLOG_CAP = 0.9, 0.10, 4.0
PROBE_CHECKPOINTS = (272, 1296, 2320)
PROBE_STEPS, SLOPE_W = 32, 8
CONFIG = [("LAW_29", 24, ["SHAM", "PARENT", "SINK_FLOOR"]),
          ("LAW_29", 32, ["SHAM", "PARENT", "SINK_FLOOR"])]
PROTOCOL, SEAL = "p08e_protocol.json", "p08e_protocol.sha256"
...
def run():
    ...
    Path("p08e_manifest.json").write_text(json.dumps(
        {"phase": "08E", "protocol_sha256": seal, "blocks": man,
         "sham_relaxation_steps": {f"{k[0]}|L{k[1]}": v for k, v in tau.items()},
         "engine_invocations": calls}, indent=1))
    for name, data in (("p08e_rows.csv", rows), ("p08e_event_ledger.csv", ev),
                       ("p08e_probe.csv", probes)):
        if not data:
            continue
        ...
        with Path(name).open("w", newline="") as h:
            ...
            w.writeheader()
            w.writerows(data)
    Path("_p08e_calls.json").write_text(json.dumps({"engine_invocations": calls}))
    print(f"\n08E: {len(rows)} trajectoires, {calls} appels moteur ({time.time()-t0:.0f}s)")
